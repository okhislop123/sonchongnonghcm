...
function showdulieu(){
	...
	if($_REQUEST['a'] == 'man'){
		if(isset($_GET['seach'])){
			...
			if($seach == 'id'){
				$items = $d->o_fet("select * from #_map where id = '".$key."'");
			}else{
				$items = $d->o_fet("select * from #_map where ten_vn like '%".$key."%'");
			}
		}
		else  $items = $d->o_fet("select * from #_map order by so_thu_tu asc");
		...
		if(isset($_REQUEST['id'])){
			...
			$items = $d->o_fet("select * from #_map where id =  '".$id."'");
		}
		$soluong = $loai = $d->o_fet("select * from #_map");
	}
}
...
function luudulieu(){
	...
	if($id != '')
	{
		...
		$d->setWhere('id',$id);
		if($d->update($data)){
			$d->redirect("index.php?p=map&a=man&page=".@$_REQUEST['page']."");
		}
		else{
			 $d->transfer("Cập nhật dữ liệu bị lỗi", "index.php?p=map&a=man");
		}
	}
	else
	{
		...
		if($d->insert($data))
		{
			$d->redirect("index.php?p=map&a=man");
		}
		else{
			 $d->transfer("Thêm dữ liệu bị lỗi!", "index.php?p=map&a=man");
		}
	}
}
...
function xoadulieu(){
	...
	if(isset($_GET['id'])){
		...
		if($d->delete()){
			...
			$d->transfer("Xóa dữ liệu bị lỗi", "index.php?p=map&a=man");
		}
	}else $d->transfer("Không nhận được dữ liệu", "index.php?p=map&a=man");
}
...
function xoadulieu_mang(){
	...
	if(isset($_POST['chk_child'])){
		...
		$hinhanh = $d->o_fet("select * from #_map where id in ($chuoi)");
		if($d->o_que("delete from #_map where id in ($chuoi)")){

			$d->redirect("index.php?p=map&a=man&page=".@$_REQUEST['page']."");
		}
		else $d->transfer("Xóa dữ liệu bị lỗi", "index.php?p=map&a=man");
	}else $d->redirect("index.php?p=map&a=man&page=".@$_REQUEST['page']."");
}
?>