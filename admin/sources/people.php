...
function showdulieu(){
	...
	if($_REQUEST['a'] == 'man'){
		...
		if(isset($_GET['lammoi'])){
			$d->o_que("update #_people set ngay_dang = '".time()."' where id = '".addslashes($_GET['id'])."'");
		}
		if(isset($_GET['loaitin']) && $_GET['loaitin'] <> ''){
			
			if($_GET['loaitin'] == 0){
				$items = $d->o_fet("select * from #_people order by so_thu_tu asc, ngay_dang desc, id desc");
			}else{
			    ...
			    $items = $d->o_fet("select * from #_people where FIND_IN_SET(id_loai,'$id_loai') <> 0 order by so_thu_tu asc, ngay_dang desc, id desc");
			}
		}else if(isset($_GET['seach'])){
			$seach = addslashes($_GET['seach']);
			$key = (isset($_GET['key']))? addslashes($_GET['key']):"";
			if($seach == 'id'){
				$items = $d->o_fet("select * from #_people where id = '".$key."'");
			}else{
				$key = $d->bodautv($key);
				$items = $d->o_fet("select * from #_people where alias_vn like '%$key%' order by so_thu_tu asc, ngay_dang desc, id desc");
			}
		}
		else $items = $d->o_fet("select * from #_people order by so_thu_tu asc, ngay_dang desc, id desc");
		...
		if(isset($_REQUEST['id'])){
			...
			$items = $d->o_fet("select * from #_people where id =  '".$id."'");
...
function luudulieu(){
	...
	if($id != '')
	{
		...
		if(@$file = $d->upload_image("file", '', '../img_data/images/',$file_name)){
			...
			$hinhanh = $d->o_fet("select * from #_people where id = '".$id."'");
		...
		if($d->update($data)){
			...
			$tintuc = $d->o_fet("select * from #_people where hien_thi = 1 order by id desc");
			...
			$d->redirect("index.php?p=people&a=man&page=".@$_REQUEST['page']."&loaitin=".@$_GET['loaitin']);
		}
		else{

			$d->alert("Cập nhật dữ liệu bị lỗi!");
			$d->redirect("Cập nhật dữ liệu bị lỗi", "index.php?p=people&a=man&loaitin=".@$_GET['loaitin']);
		}
	}
	else
	{
...
error_reporting(E_ALL);
		...
		if($idsp = $d->insert($data))
		{
			...
			$d->redirect("index.php?p=people&a=man&loaitin=".@$_GET['loaitin']);
		}
		else{
			$d->alert("Thêm dữ liệu bị lỗi!");
			$d->redirect("Thêm dữ liệu bị lỗi", "index.php?p=people&a=man&loaitin=".@$_GET['loaitin']);
		}
	}
}
...
function xoadulieu(){
	...
	if(isset($_GET['id'])){
		...
		if($d->delete()){
			...
			$d->redirect("Xóa dữ liệu bị lỗi", "index.php?p=people&a=man&loaitin=".@$_GET['loaitin']);
		}
	}else {
...
function xoadulieu_mang(){
	...
	if(isset($_POST['chk_child'])){
		...
		$hinhanh = $d->o_fet("select * from #_people where id in ($chuoi)");
		if($d->o_que("delete from #_people where id in ($chuoi)")){
			//xoa hình ảnh
			foreach ($hinhanh as $ha) {
				@unlink('../img_data/images/'.$ha['hinh_anh']);
	...
	}else $d->redirect("index.php?p=people&a=man&page=".@$_REQUEST['page']."&loaitin=".@$_GET['loaitin']);
}
?>