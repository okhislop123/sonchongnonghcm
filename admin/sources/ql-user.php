...
function showdulieu(){
	...
	if($_REQUEST['a'] == 'man'){
		...
		if(isset($_GET['seach'])){
                    ...
                    $items = $d->o_fet("select * from #_user where id <> ".$_SESSION['id_user']." and (user_ql=".$_SESSION['id_user']." or user_tao= ".$_SESSION['id_user'].") order by id desc");
                }
...
function luudulieu(){
	...
	if($id != '')
	{
            ...
            $user_login = $d->o_fet("select * from #_user where id= ". $_SESSION['id_user']." ");
            $data['tai_khoan'] = $d->clean(addslashes($_POST['tai_khoan']));
            $data['user_hash'] = sha1($data['tai_khoan']);
            $data['ho_ten'] = addslashes($_POST['ho_ten']);
            $data['pass_hash'] = sha1($d->clean(addslashes($_POST['password'])));
            ...
            $data['user_tao'] = $_SESSION['id_user'];
            if($user_login[0]['user_ql']==0){
                $data['user_ql'] = $_SESSION['id_user'];
            }else{
                $data['user_ql'] = $user_login[0]['user_ql'];
            }
