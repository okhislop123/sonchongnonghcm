...
function luudulieu()
{
	...
	if ($id != '') {
		...
		if (@$document = $d->upload_image("doc", '', '../img_data/images/', $file_name)) {
			$hinhanh = $d->o_fet("select * from #_sanpham where id = '" . $id . "'");
			// unlink('../img_data/images/' . $hinhanh[0]['doc']);
			$data['doc'] = $document;
		...
		if ($_POST['detetefile']) {
			$data['doc'] = "";
			unlink('../img_data/images/' . $document);
		}
		...
		if ($_POST['sizesValue'] != null && $_POST['sizesValue'] && $_POST['sizesValue'] != "[]") {
			$data['size'] = $_POST['sizesValue'];
		} else {
			$data['size'] = "";
		}
		...
		if ($_POST['sizesValue'] != null && $_POST['sizesValue']) {
			$data['size'] = $_POST['sizesValue'];
		} else {
			$data['size'] = "";
		}
