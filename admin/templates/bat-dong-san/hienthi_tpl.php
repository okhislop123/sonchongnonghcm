...
<div class="form-group">
	...
	<div class="btn-group">
		<select id="action" onchange="seach(this,'bat-dong-san')" name="action" class="form-control">
	...
	<div class="btn-group">
		<select id="action" onchange="show(this,'bat-dong-san')" name="action" class="form-control">
	...
	<div class="btn-group">
		<select id="action" onchange="loc_tin(this,'bat-dong-san')" name="action" class="form-control">
			<option value="0" selected>Xem tất cả bất động sản</option>
			<?=$loai?>
		</select>
	</div>
	<a href="index.php?p=bat-dong-san&a=add" class="btn btn-primary pull-right"><i class="glyphicon glyphicon-plus"></i> Thêm mới</a>
</div>

<form id="form" method="post" action="index.php?p=bat-dong-san&a=delete_all" role="form">
...
<table class="table table-bordered table-hover">
	...
	<tbody>
		...
		<tr>
			...
			<td style="text-align:left">
				<a href="index.php?p=bat-dong-san&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=$items[$i]['ten_vn']?></a>
				<br/>
				<a href="index.php?p=bat-dong-san&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=$items[$i]['ten_us']?></a>
				<br/>
				<a href="index.php?p=bat-dong-san&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=$items[$i]['ten_ch']?></a>
			</td>
			</td>
			<td>
				<a href="index.php?p=bat-dong-san&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=($items[$i]['hinh_anh'] <> '')?"<img src='".URLPATH."thumb.php?src=".URLPATH."img_data/images/".$items[$i]['hinh_anh']."&w=70&h=50'>":""; ?></a>
			</td>
			...
			<td>
				<a href="index.php?p=bat-dong-san&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" class="text-danger" title="Sửa"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;&nbsp;&nbsp;
				<a href="index.php?p=bat-dong-san&a=delete&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" onClick="if(!confirm('Xác nhận xóa?')) return false;" class="text-danger" title="Xóa"><i class="glyphicon glyphicon-remove"></i></a>
			</td>
		</tr>
		<?php } ?>
	</tbody>
