<ol class="breadcrumb">
  ...
  <li class="active"><a href="<?=urladmin ?>index.php?p=<?=$_GET['p'] ?>&a=man">Hãng sản phẩm</a></li>
</ol>
...
<table class="table table-bordered table-hover">
	<thead>
		<tr>
			...
			<th style="width:70%; text-align:left">Tiêu đề</th>
	...
	<tbody>
		...
		<tr>
			...
			<td style=" text-align:left">
				...
				<br/>
				 <a href="index.php?p=<?=$_GET['p'] ?>&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=$items[$i]['ten_us']?></a>
			</td>

			<td>
				<input class="chk_box" type="checkbox" onclick="on_check(this,'#_hangsanpham','hien_thi','<?=$items[$i]['id']?>')" <?php if($items[$i]['hien_thi'] == 1) echo 'checked="checked"'; ?>>
			</td>
			<td>
