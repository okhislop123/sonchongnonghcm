...
<div class="form-group">
	...
	<div class="btn-group">
		<select id="action" onchange="seach(this,'hinh-thuc-thanh-toan')" name="action" class="form-control">
	...
	<div class="btn-group">
		<select id="action" onchange="show(this,'hinh-thuc-thanh-toan')" name="action" class="form-control">
	...
	<a href="index.php?p=hinh-thuc-thanh-toan&a=add" class="btn btn-primary pull-right"><i class="glyphicon glyphicon-plus"></i> Thêm mới</a>
</div>

<form id="form" method="post" action="index.php?p=hinh-thuc-thanh-toan&a=delete_all" role="form">
...
<table class="table table-bordered table-hover">
	<thead>
		<tr>
			...
			<th style="width:76%; text-align:left">Tiêu đề</th>
	...
	<tbody>
		...
		<tr>
			...
			<td style=" text-align:left">
				<a href="index.php?p=hinh-thuc-thanh-toan&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=$items[$i]['ten_vn']?></a> 
				<br/>
				 <a href="index.php?p=hinh-thuc-thanh-toan&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=$items[$i]['ten_us']?></a>
			</td>
			...
			<td>
				<a href="index.php?p=hinh-thuc-thanh-toan&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" class="text-danger" title="Sửa"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;&nbsp;&nbsp;
				<a href="index.php?p=hinh-thuc-thanh-toan&a=delete&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" onClick="if(!confirm('Xác nhận xóa?')) return false;" class="text-danger" title="Xóa"><i class="glyphicon glyphicon-remove"></i></a>
			</td>
		</tr>
		<!-- // cap 1 -->
