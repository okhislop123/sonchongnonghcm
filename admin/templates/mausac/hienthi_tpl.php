<ol class="breadcrumb">
  ...
  <li class="active"><a href="<?=urladmin ?>index.php?p=mausac&a=man">Kích thước</a></li>
</ol>
...
<div class="form-group">
	...
	<div class="btn-group">
		<select id="action" onchange="seach(this,'mausac')" name="action" class="form-control">
	...
	<div class="btn-group">
		<select id="action" onchange="show(this,'mausac')" name="action" class="form-control">
	...
	<a href="index.php?p=mausac&a=add" class="btn btn-primary pull-right"><i class="glyphicon glyphicon-plus"></i> Thêm mới</a>
</div>

<form id="form" method="post" action="index.php?p=mausac&a=delete_all" role="form">
	...
	<tbody>
		...
		<tr>
			...
			<td style=" text-align:left">
				<a href="index.php?p=mausac&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=$items[$i]['ten_vn']?></a> 

			</td>

			<td>
				<input class="chk_box" type="checkbox" onclick="on_check(this,'#_mausac','hien_thi','<?=$items[$i]['id']?>')" <?php if($items[$i]['hien_thi'] == 1) echo 'checked="checked"'; ?>>
			</td>
			<td>
				<a href="index.php?p=mausac&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" class="text-danger" title="Sửa"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;&nbsp;&nbsp;
				<a href="index.php?p=mausac&a=delete&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" onClick="if(!confirm('Xác nhận xóa?')) return false;" class="text-danger" title="Xóa"><i class="glyphicon glyphicon-remove"></i></a>
			</td>
		</tr>
