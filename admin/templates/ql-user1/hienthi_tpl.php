...
<div class="form-group tac-vu">
	...
	<div class="btn-group">
		<select id="action" onchange="seach(this,'ql-user1')" name="action" class="form-control">
	...
	<div class="btn-group">
		<select id="action" onchange="show(this,'ql-user1')" name="action" class="form-control">
			...
			<td>
                ...
                <option <?php if(@$items[$i]['quyen_han'] == 3) echo "selected='selected'";?> value="2">Quản trị viên cấp 2</option>
                	...
                	<P><?php if(@$items[$i]['quyen_han'] == 0) echo "user";?></P>
				 	...
				 	<P><?php if(@$items[$i]['quyen_han'] == 1) echo "User";?></P>

			</td>
			<td>
				<?php if($items[$i]['hien_thi'] == 1){ ?>
				<a href="index.php?p=ql-user1&a=man&b=hien_thi&TT=0&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" class="text-danger"><font style="color:#428bca">Hiện</font></a>
				<?php }else{ ?>
				<a href="index.php?p=ql-user1&a=man&b=hien_thi&TT=1&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" class="text-danger"><font style="color:red">Ẩn</font></a>
				<?php } ?>
			</td>
			<td>
				<a href="index.php?p=ql-user1&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" class="text-danger" title="Sửa"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;&nbsp;&nbsp;
				<a href="index.php?p=ql-user1&a=delete&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" onClick="if(!confirm('Xác nhận xóa?')) return false;" class="text-danger" title="Xóa"><i class="glyphicon glyphicon-remove"></i></a>
			</td>
		</tr>
		<?php } ?>
	</tbody>
