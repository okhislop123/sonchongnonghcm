...
<ol class="breadcrumb">
  ...
  <li class="active"><a href="<?=urladmin ?>index.php?p=quan&a=man">Quận</a></li>
  <li class="active"><a href="#"><?php if(isset($_GET['id'])) echo "Sửa "; else echo "Thêm mới" ?></a></li>
</ol>
<div class="col-xs-12">
<form name="frm" method="post" action="index.php?p=quan&a=save&id=<?=@$_REQUEST['id']?>&page=<?=@$_REQUEST['page']?>" enctype="multipart/form-data">
		...
		<table class="table table-bordered table-hover them_dt" style="border:none">
			<tbody>
				<tr>
					...
					<td class="td_right">
						<select name="id_loai" class="input width400 form-control" style="border-radius:4px">
		    				<?php 
		    					$thanhpho = $d->o_sel("*","#_thanhpho","hien_thi = 1","so_thu_tu asc");
		    					foreach ($thanhpho as $lsp) {
		    				?>
				...
				<tr>
					...
					<td class="td_right">
						<input type="button" value="Thoát" onclick="javascript:window.location='index.php?p=quan&a=man'" class="btn btn-primary" />
					</td>
				</tr>
			</tbody>
		</table>
