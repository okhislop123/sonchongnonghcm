<ol class="breadcrumb">
  ...
  <li class="active"><a href="<?=urladmin ?>index.php?p=thanh-pho&a=man">Thành phố</a></li>
</ol>
...
<div class="form-group">
	...
	<div class="btn-group">
		<select id="action" onchange="seach(this,'thanh-pho')" name="action" class="form-control">
	...
	<div class="btn-group">
		<select id="action" onchange="show(this,'thanh-pho')" name="action" class="form-control">
	...
	<a href="index.php?p=thanh-pho&a=add" class="btn btn-primary pull-right"><i class="glyphicon glyphicon-plus"></i> Thêm mới</a>
</div>

<form id="form" method="post" action="index.php?p=thanh-pho&a=delete_all" role="form">
...
<table class="table table-bordered table-hover">
	<thead>
		<tr>
			...
			<th style="width:76%; text-align:left">Danh mục</th>
	...
	<tbody>
		...
		<tr>
			...
			<td style=" text-align:left">
				<a href="index.php?p=thanh-pho&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>"><?=$items[$i]['ten_vn']?></a> 

			</td>

			<td>
				<input class="chk_box" type="checkbox" onclick="on_check(this,'#_thanhpho','hien_thi','<?=$items[$i]['id']?>')" <?php if($items[$i]['hien_thi'] == 1) echo 'checked="checked"'; ?>>
			</td>
			<td>
				<a href="index.php?p=thanh-pho&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" class="text-danger" title="Sửa"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;&nbsp;&nbsp;
				<a href="index.php?p=thanh-pho&a=delete&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>" onClick="if(!confirm('Xác nhận xóa?')) return false;" class="text-danger" title="Xóa"><i class="glyphicon glyphicon-remove"></i></a>
			</td>
		</tr>
